...
export default function ColorCustomizationAdmin() {
  ...
  const { mutate: setTextColor, isPending: settingTextColor } = useSetTextColor();
  const { mutate: setHeaderTextColor, isPending: settingHeaderTextColor } = useSetHeaderTextColor();
  const { mutate: setAccentColor, isPending: settingAccentColor } = useSetAccentColor();
  const { mutate: setLandingPageBg, isPending: settingLandingBg } = useSetLandingPageBackgroundColor();
  const { mutate: setApartmentsPageBg, isPending: settingApartmentsBg } = useSetApartmentsPageBackgroundColor();
  const { mutate: setAuraCollectionPageBg, isPending: settingAuraCollectionBg } = useSetAuraCollectionPageBackgroundColor();
  const { mutate: setApartmentDetailPageBg, isPending: settingApartmentDetailBg } = useSetApartmentDetailPageBackgroundColor();
  const { mutate: setArtistDetailPageBg, isPending: settingArtistDetailBg } = useSetArtistDetailPageBackgroundColor();
  const { mutate: setArtworkDetailPageBg, isPending: settingArtworkDetailBg } = useSetArtworkDetailPageBackgroundColor();
  ...
  const [auraCollectionPageBg, setAuraCollectionPageBgLocal] = useState('#FFFFFF');
  const [apartmentDetailPageBg, setApartmentDetailPageBgLocal] = useState('#FFFFFF');
  const [artistDetailPageBg, setArtistDetailPageBgLocal] = useState('#FFFFFF');
  const [artworkDetailPageBg, setArtworkDetailPageBgLocal] = useState('#FFFFFF');
  ...
  useEffect(() => {
    if (siteConfig) {
      ...
      setApartmentsPageBgLocal(siteConfig.apartmentsPageBackgroundColor || '#FFFFFF');
      setAuraCollectionPageBgLocal(siteConfig.auraCollectionPageBackgroundColor || '#FFFFFF');
      setApartmentDetailPageBgLocal(siteConfig.apartmentDetailPageBackgroundColor || '#FFFFFF');
      setArtistDetailPageBgLocal(siteConfig.artistDetailPageBackgroundColor || '#FFFFFF');
      setArtworkDetailPageBgLocal(siteConfig.artworkDetailPageBackgroundColor || '#FFFFFF');
    }
  }, [siteConfig]);
  ...
  const handleSaveApartmentsPageBg = () => {
    setApartmentsPageBg(apartmentsPageBg, {
      onSuccess: () => toast.success('Apartments page background color updated'),
  ...
  const handleSaveAuraCollectionPageBg = () => {
    setAuraCollectionPageBg(auraCollectionPageBg, {
      onSuccess: () => toast.success('Aura Collection page background color updated'),
      onError: () => toast.error('Failed to update Aura Collection page background'),
    });
  };

  const handleSaveApartmentDetailPageBg = () => {
    setApartmentDetailPageBg(apartmentDetailPageBg, {
      onSuccess: () => toast.success('Apartment detail page background color updated'),
      onError: () => toast.error('Failed to update apartment detail page background'),
    });
  };

  const handleSaveArtistDetailPageBg = () => {
    setArtistDetailPageBg(artistDetailPageBg, {
      onSuccess: () => toast.success('Artist detail page background color updated'),
      onError: () => toast.error('Failed to update artist detail page background'),
    });
  };

  const handleSaveArtworkDetailPageBg = () => {
    setArtworkDetailPageBg(artworkDetailPageBg, {
      onSuccess: () => toast.success('Artwork detail page background color updated'),
      onError: () => toast.error('Failed to update artwork detail page background'),
    });
  };
    ...
    <div className="space-y-6">
      <Tabs defaultValue="text" className="space-y-6">
        <TabsList className="grid w-full grid-cols-3 bg-luxury-dark/50 border border-luxury-gold/20">
          <TabsTrigger value="text" className="data-[state=active]:bg-luxury-gold/20 data-[state=active]:text-white text-white/70">
            Text Colors
          </TabsTrigger>
          <TabsTrigger value="accent" className="data-[state=active]:bg-luxury-gold/20 data-[state=active]:text-white text-white/70">
            Accent Color
          </TabsTrigger>
          <TabsTrigger value="backgrounds" className="data-[state=active]:bg-luxury-gold/20 data-[state=active]:text-white text-white/70">
            Page Backgrounds
          </TabsTrigger>
        </TabsList>
          ...
          <Card className="border-luxury-gold/20 bg-luxury-dark/50">
            <CardHeader>
              ...
              <CardDescription className="text-white/70">
                Customize normal text and header text colors separately for enhanced typography control
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              {/* Header Text Color */}
              <div className="space-y-3">
                <Label htmlFor="headerTextColor" className="text-white">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) => setHeaderTextColorLocal(e.target.value)}
                        className="h-12 w-20 cursor-pointer border-luxury-gold/30 bg-white/5"
                      />
                      <Input
                        type="text"
        ...
        <TabsContent value="accent">
          <Card className="border-luxury-gold/20 bg-luxury-dark/50">
            <CardHeader>
              ...
              <CardDescription className="text-white/70">
                Customize the accent color used for buttons, links, and interactive elements
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-3">
        ...
        <TabsContent value="backgrounds">
          <Card className="border-luxury-gold/20 bg-luxury-dark/50">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                <Label className="text-white">Landing Page Background</Label>
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    className="bg-luxury-gold text-luxury-dark hover:bg-luxury-gold/90"
                  >
                    {settingLandingBg ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Label className="text-white">Apartments Page Background</Label>
                <div className="flex gap-3">
                  <div className="flex-1 flex gap-2">
                    <Input
                      type="color"
                  ...
                  <Button
                    ...
                    className="bg-luxury-gold text-luxury-dark hover:bg-luxury-gold/90"
                  >
                    {settingApartmentsBg ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Label className="text-white">Aura Collection Page Background</Label>
                <div className="flex gap-3">
                  <div className="flex-1 flex gap-2">
                    <Input
                      type="color"
                      value={auraCollectionPageBg}
                      onChange={(e) => setAuraCollectionPageBgLocal(e.target.value)}
                      className="h-12 w-20 cursor-pointer border-luxury-gold/30 bg-white/5"
                    />
                    <Input
                      type="text"
                  ...
                  <Button
                    ...
                    className="bg-luxury-gold text-luxury-dark hover:bg-luxury-gold/90"
                  >
                    {settingAuraCollectionBg ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Label className="text-white">Apartment Detail Page Background</Label>
                <div className="flex gap-3">
                  <div className="flex-1 flex gap-2">
                    <Input
                      type="color"
                      value={apartmentDetailPageBg}
                      onChange={(e) => setApartmentDetailPageBgLocal(e.target.value)}
                      className="h-12 w-20 cursor-pointer border-luxury-gold/30 bg-white/5"
                    />
                    <Input
                      type="text"
                  ...
                  <Button
                    ...
                    className="bg-luxury-gold text-luxury-dark hover:bg-luxury-gold/90"
                  >
                    {settingApartmentDetailBg ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Label className="text-white">Artist Detail Page Background</Label>
                <div className="flex gap-3">
                  <div className="flex-1 flex gap-2">
                    <Input
                      type="color"
                      value={artistDetailPageBg}
                      onChange={(e) => setArtistDetailPageBgLocal(e.target.value)}
                      className="h-12 w-20 cursor-pointer border-luxury-gold/30 bg-white/5"
                    />
                    <Input
                      type="text"
                  ...
                  <Button
                    ...
                    className="bg-luxury-gold text-luxury-dark hover:bg-luxury-gold/90"
                  >
                    {settingArtistDetailBg ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <Label className="text-white">Artwork Detail Page Background</Label>
                <div className="flex gap-3">
                  <div className="flex-1 flex gap-2">
                    <Input
                      type="color"
                      value={artworkDetailPageBg}
                      onChange={(e) => setArtworkDetailPageBgLocal(e.target.value)}
                      className="h-12 w-20 cursor-pointer border-luxury-gold/30 bg-white/5"
                    />
                    <Input
                      type="text"
                  ...
                  <Button
                    ...
                    className="bg-luxury-gold text-luxury-dark hover:bg-luxury-gold/90"
                  >
                    {settingArtworkDetailBg ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save'}
                  </Button>
                </div>
              </div>
            </CardContent>
      ...
      <Card className="border-luxury-gold/20 bg-luxury-dark/50">
        <CardContent className="flex justify-between items-center p-6">
          <div>
            ...
            <p className="text-sm text-white/60">Reset all color settings to default values</p>
          </div>
          <Button
      ...
      <Card className="border-luxury-gold/20 bg-luxury-dark/50">
        ...
        <CardContent className="space-y-6">
          <div className="rounded-lg border border-white/10 p-8 space-y-6" style={{ backgroundColor: landingPageBg }}>
            <div>
              <h1 
              ...
              >
                This is how your body text will appear throughout the site. The normal text color you choose will be applied to all content.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="h-1 w-24 rounded" style={{ backgroundColor: accentColor }} />
            </div>
          </div>

          <p className="text-sm text-white/60 text-center">
            Remember to click "Publish Changes" in the header to make these colors live on your public site
          </p>
        </CardContent>
      </Card>
    </div>
