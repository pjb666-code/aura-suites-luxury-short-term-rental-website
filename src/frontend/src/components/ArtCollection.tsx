...
const FALLBACK_COLLECTION_IMAGE =
  "/assets/pjbtps_modern_art_painting_inspired_by_buenos_aires_and_latin_71d0b888-e7c6-47ac-940c-80bbde9cc543_1.png";

function CollectionMainImage({
  imageKey,
  fallback,
  accentColor,
  onClick,
  ...
  const { data: uploadedUrl } = useFileUrl(imageKey ?? "");
  const src = imageKey ? (uploadedUrl ?? fallback) : fallback;

  return (
    <button
      type="button"
      className="group relative mx-auto block w-full max-w-6xl cursor-pointer overflow-hidden rounded-lg shadow-2xl transition-all duration-300 hover:shadow-3xl"
      style={{ backgroundColor: "transparent", border: "none", padding: 0 }}
      onClick={onClick}
      aria-label="View the full Aura Collection"
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-all duration-300 group-hover:bg-black/10">
        <div className="translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <span
            className="inline-flex items-center gap-2 rounded-md px-6 py-3 text-base font-medium shadow-lg"
    ...
    >
      <div className="container mx-auto px-4">
        ...
        <div className="mb-20">
          <CollectionMainImage
            imageKey={
              siteConfig?.auraCollectionSection?.auraCollectionMainImageKey ??
              undefined
            }
            fallback={FALLBACK_COLLECTION_IMAGE}
            accentColor={accentColor}
            onClick={() => navigate({ to: "/aura-collection" })}
          />
          <p
            className="mt-6 text-center text-sm italic opacity-70"
            style={{ color: textColor }}
          >
            {sectionTexts?.featuredText ||
        ...
        {!isLoading && featuredArtists.length > 0 && (
          <div className="mb-20">
            <h3
              className="mb-8 flex items-center justify-center gap-2 text-2xl font-light"
              style={{ color: textColor }}
            >
                ...
                >
                  ...
                  >
                    ...
                    <CardContent className="p-6">
                      ...
                      <div
                        className="mt-4 flex items-center text-sm font-medium group-hover:gap-2 transition-all"
